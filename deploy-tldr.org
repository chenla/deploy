...
because it is simular to, but not the same as the older term
"executive summary", "synopsis" or "scope note" which are
themselves all slightly different concepts.
...
to rabbit holes which help you descide if you wish to tumble
down into or not.

In a very real sense -- the Chenla Institute's primary
activity is creating entrances to a myriad of rabbit holes
that taken together constitute a distributed technological
civilization.


 - Chenla TL;DRs

  - these are stubs that frame an idea or project
  ...
  - each TL;DR is an imutable namespace so that the TL;DR as
    a concept is imutable, but the expressions,
    manifestations and instances of the TL;RD are mutable
    over time.




Chenla will publish these online as they are complete and
collected together into an annual volume that will be
printed as a physical book

followed up by and expanded as in-depth proposals,
narratives, research, code, designs etc.
