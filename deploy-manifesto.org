...
There is no fate that determines the future, there is only
complexity that allows us to understand where things are
headed in general, but not specifically.  There is nothing
to petition or pray to that can change that. We must live as
individuals with uncertainty, but over time scales that are
beyond living memory we can collectively pass the test and
survive the filter.
